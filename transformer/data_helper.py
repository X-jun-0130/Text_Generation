...
class TransformerSeq2SeqData(object):
    ...
    def padding(self, batch):
        ...
        decoder_length = [len(response) for response in responses]
        max_decoder_length = max(decoder_length)

        # 按最大长度补齐
        decoder_inputs = [response + [self.pad_token] * (max_decoder_length - len(response))
                          for response in responses]
 

        return np.array(encoder_inputs, dtype=np.int32), np.array(decoder_inputs,dtype=np.int32)

    def gen_data(self, file_path, is_training=False):
        """
        生成可导入到模型中的数据
        :param file_path
        :param is_training
    ...
    def next_batch(self, data, batch_size):
        ...
        for i in range(batch_num):
            ...
            encoder_inputs, decoder_inputs = self.padding(batch_data)
            yield encoder_inputs, decoder_inputs
